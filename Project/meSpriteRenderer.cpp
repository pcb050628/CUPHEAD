...
namespace me
{
	SpriteRenderer::SpriteRenderer(GameObject* gobj) : Component(enums::eComponentType::SpriteRenderer, gobj)
		, mTexture(nullptr)
	...
	void SpriteRenderer::Render(HDC hdc)
	{
		...
		if (mTexture != nullptr)
		{
			...
			{
				TransparentBlt(hdc, pos.x - (mTexture->GetWidth() * mScale.x / 2), pos.y - (mTexture->GetHeight() * mScale.y / 2)
			...
			else if (mTexture->GetType() == eTextureType::png)
			{
				...
				Gdiplus::Rect rect = { (int)(pos.x - ((mWidht * mScale.x) / 2)), (int)(pos.y - ((mHeight * mScale.y) / 2))
					, (int)(mWidht * mScale.x), (int)(mHeight * mScale.y) };
